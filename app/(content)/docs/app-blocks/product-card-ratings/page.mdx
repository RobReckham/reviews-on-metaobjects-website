...
```liquid
<script class="fo-product-card-rating-{{ card_product.id }}">fiveoh.mountProductCardRating({ productId: {{ card_product.id | json }}, ratingValue: {{ card_product.metafields.reviews.rating.value.rating | json }}, ratingCount: {{ card_product.metafields.reviews.rating_count.value | json }} });</script>
```
