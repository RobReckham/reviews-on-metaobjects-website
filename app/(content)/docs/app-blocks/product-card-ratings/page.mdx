...
```liquid
<script class="fo-product-card-rating-{{ card_product.id }}">document.dispatchEvent(new CustomEvent("fiveoh:product-card-rating-mounted", { detail: { product_id: {{ card_product.id | json }}, rating_value: {{ card_product.metafields.reviews.rating.value.rating | json }}, rating_count: {{ card_product.metafields.reviews.rating_count.value | json }}}}));</script>
```
