...
export default function Modal({ isOpen, onClose, children, className = '' }) {
  ...
  return <>
    <div className={`fixed z-50 inset-0 bg-black opacity-80 ${isOpen ? 'block' : 'hidden'}`} onClick={onClose}></div>
    <div className={`fixed z-50 inset-0 flex justify-center items-center p-4 pointer-events-none ${isOpen ? 'block' : 'hidden'}`}>
      <div className={`bg-white p-8 rounded-lg w-full max-w-xl pointer-events-auto relative text-left overflow-y-auto text-base font-normal tracking-normal text-gray-900 max-h-[90vh] ${className}`}>
        {children}
