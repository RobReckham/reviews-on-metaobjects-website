...
export default function Faqs() {
  ...
  return <ul className="space-y-8 mt-16">
    {faqs.map((faq, index) => <li key={index} className="bg-white rounded-2xl">
      <button className="flex justify-between items-center w-full text-left p-8 cursor-pointer" onClick={() => setActiveFaq(index === activeFaq ? null : index)}>
        <h3 className="h3 cursor-pointer">{faq.question}</h3>
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className={`size-8 text-black transition-transform duration-300 ${activeFaq === index ? 'rotate-180' : ''}`}>
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={6} d="M19.5 8.25l-7.5 7.5-7.5-7.5" />
        </svg>
      </button>
      <div className="grid transition-all duration-300 ease-in-out" style={{ gridTemplateRows: activeFaq === index ? '1fr' : '0fr' }}>
        <div className="overflow-hidden">
          <div className="text-xl p-8 pt-0 flex flex-col gap-4">
            {faq.answer}
          </div>
        </div>
      </div>
